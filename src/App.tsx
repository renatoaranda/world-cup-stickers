...
export default function App() {
    ...
    if (saved) {
      try {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          ...
          const hasInvalidTeam = parsed.some((s) => !TEAMS.some((t) => t.id === s.teamId));
        ...
        console.error('Falha ao ler dados do localStorage. Iniciando padrão.', err);
        setStickers(generateDefaultStickers());
      }
    } else {
      setStickers(generateDefaultStickers());
  ...
  useEffect(() => {
    const activeBtn = document.getElementById(`global-shortcut-${activeScrollTeam}`);
    ...
    if (activeBtn && ribbon) {
      ...
      const targetScrollLeft = offsetLeft - (ribbonRect.width / 2) + (activeRect.width / 2);
  ...
  const totalDuplicatesCount = stickers.reduce((sum, s) => sum + s.duplicates, 0);
  const completionPercent = totalStickersCount > 0 
  ...
  const handleExportCSV = () => {
    try {
      ...
      downloadCSV(csvContent, `album_copa_progresso_${completionPercent}percent.csv`);
      addToast('Exportação concluída! Verifique sua pasta de downloads. 📥', 'success');
    } catch (err) {
      console.error(err);
      addToast('Infelizmente, ocorreu um erro ao gerar a exportação CSV.', 'error');
    }
  };

  // 8. CSV Import Apply
  const handleApplyImport = (importedData: Partial<Sticker>[], mode: 'merge' | 'replace') => {
    let baseStickers = mode === 'replace' ? generateDefaultStickers() : [...stickers];
    ...
    const updated = baseStickers.map((sticker) => {
      ...
      const match = importedData.find(
        (csvCell) => 
          (csvCell.id && csvCell.id.toLowerCase() === sticker.id.toLowerCase()) ||
          (csvCell.code && csvCell.code.replace(/\s+/g, '').toLowerCase() === sticker.code.replace(/\s+/g, '').toLowerCase())
      );
      ...
      if (match) {
        return {
          ...
          duplicates: match.duplicates !== undefined ? match.duplicates : sticker.duplicates,
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 selection:bg-amber-400/20 selection:text-amber-100 relative overflow-x-clip" id="main-application-viewport">
      
      {/* 🔮 Mesh gradient styling background */}
      <div className="mesh-bg"></div>

      {/* 🏆 HERO HEADER HEADER ROW (STICKY) */}
      <header className="sticky top-0 z-40 bg-[#070b19]/95 backdrop-blur-md border-b border-white/5 shadow-xl select-none" id="main-app-header">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 md:py-3.5 relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="p-2 bg-amber-500 text-slate-950 rounded-xl shadow-md flex items-center justify-center select-none animate-pulse-subtle" id="logo-icon-container">
                <Trophy className="w-5 h-5 font-black stroke-[2.5]" />
              </div>
              <div>
                <h1 className="text-xl md:text-2xl font-black tracking-tight text-white leading-none" id="app-title-header">
                  Álbum de Figurinhas
                </h1>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <div className="flex items-center gap-2.5 bg-rose-500/10 border border-rose-500/20 rounded-xl px-3 py-1.5 select-none shadow-sm" id="header-missing-indicator-small">
                <div className="flex flex-col text-right">
                  <span className="text-[8px] font-bold uppercase tracking-wider text-slate-400 leading-none">Faltas</span>
                  <span className="text-xs font-extrabold text-rose-400 leading-none mt-1">{stats.missing} <span className="text-[8px] font-mono text-slate-500">/ {stats.total}</span></span>
                </div>
        ...
        <div className="border-t border-white/5 bg-slate-900/40 py-1.5 md:py-2 relative z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex overflow-x-auto md:flex-wrap gap-1.5 pb-1.5 md:pb-0 scrollbar-none snap-x justify-start" id="global-shortcuts-ribbon">
              {TEAMS.map((team) => {
        ...
        <div className="mb-2.5 md:mb-4 bg-white/5 border border-white/5 rounded-xl py-2 md:py-2.5 px-3 md:px-4 flex flex-col md:flex-row md:items-center justify-between gap-2.5 md:gap-4">
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-slate-300 font-bold leading-relaxed">
              Como usar: Toque nas figurinhas para marcar ou desmarcar do álbum. No menu flutuante do topo, use as abreviações rápidas para rolar a tela diretamente até a seleção desejada!
            </p>
          </div>
          <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs text-slate-300">
                <div className="space-y-2">
                  <h5 className="font-extrabold text-white flex items-center gap-1.5">
                    <span className="p-1 px-2 rounded bg-white/10 text-amber-400 font-mono">1</span>
                    Persistência e Auto-salvamento
                  </h5>
                  <p className="leading-relaxed text-slate-400">
                     Cada alteração feita clicando nas figurinhas salva seu progresso na memória local (localStorage) de seu navegador. Você pode recarregar ou retornar para continuar sempre de onde parou.
                  </p>
                </div>
                <div className="space-y-2">
                  <h5 className="font-extrabold text-white flex items-center gap-1.5">
                    <span className="p-1 px-2 rounded bg-white/10 text-amber-400 font-mono">2</span>
                    Importação e Mesclagem Inteligente
                  </h5>
                  <p className="leading-relaxed text-slate-400">
                     Ao fazer o upload (CSV), você pode optar por <strong>Mesclar Dados</strong>. Isso evita perder seu progresso atual: as figurinhas contidas no arquivo serão unidas com as que você já possui. Ou use Sobrescrever para carregar o estado idêntico ao do CSV.
                  </p>
                </div>
                <div className="space-y-2">
                  <h5 className="font-extrabold text-white flex items-center gap-1.5">
                    <span className="p-1 px-2 rounded bg-white/10 text-amber-400 font-mono">3</span>
                    Estrutura de Planilha Compatível
                  </h5>
                  <p className="leading-relaxed text-slate-400">
                     Você pode gerar seu próprio arquivo usando Excel ou Google Sheets. Certifique-se de que a planilha possui colunas intituladas como <code>id</code> ou <code>code</code>, além de <code>owned</code> (usar 1 para obtido).
                  </p>
                </div>
              </div>
            </motion.div>
      ...
      {/* 🔮 FOOTER BAR SECTION */}
      <footer className="bg-slate-950/40 border-t border-white/5 text-slate-400 text-xs py-8 mt-12 shrink-0 select-none relative z-10 font-sans" id="main-app-footer">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center space-y-4">
          <div className="flex justify-center items-center gap-1 font-bold text-slate-300">
            <span>Desenvolvido com carinho para os colecionadores de figurinhas do álbum da Copa do Mundo</span>
            <Heart className="w-4.5 h-4.5 text-rose-500 fill-rose-500" />
          </div>
          <div className="text-slate-500 max-w-lg mx-auto leading-relaxed">
            Este aplicativo é privado e seguro. Seus dados não são enviados para nenhum servidor ou nuvem corporativa; eles permanecem 100% gravados em localStorage no seu dispositivo local. Faça backups periódicos através do botão "Exportar CSV".
          </div>
          <div className="text-[10px] text-slate-600 font-mono">
            {new Date().getFullYear()} • Controle de Figurinhas v2.2.0 • Totalmente sem anúncios
          </div>
        </div>
      </footer>
      ...
      <AnimatePresence>
        {showResetConfirm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 py-8" id="reset-confirm-modal">
            {/* Backdrop */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.6 }}
            ...
            >
              ...
              <h4 className="text-lg font-black text-white">Tem certeza que deseja zerar o álbum?</h4>
              <p className="text-xs text-slate-400 mt-2 leading-relaxed">
                Esta ação irá marcar todas as 660 figurinhas como "Não obtidas", remover todos os nomes de jogadores cadastrados e limpar quaisquer figurinhas repetidas salvas no banco de dados local.
              </p>
